...
function highlightTerms(text: string, query: string): React.ReactNode {
	...
	try {
		...
		return parts.map((part, i) => part.toLowerCase() === lowerEscaped ? <mark key={i} className="bg-warning/20 text-foreground rounded-sm px-0.5">{part}</mark> : part);
	} catch { return text; }
}
		...
		>
			<>
				<div className="flex items-center gap-2 px-4 py-3.5 border-b border-border shrink-0 md:px-5">
					<Tooltip>
						<TooltipTrigger render={<Button variant="ghost" size="icon-sm" onClick={() => navigate(`/mailbox/${mailboxId}/emails/inbox`)} aria-label="Back to inbox" />}>
							<ArrowLeftIcon size={18} aria-hidden="true" />
						</TooltipTrigger>
						<TooltipPopup side="bottom">Back to inbox</TooltipPopup>
					</Tooltip>
					<div className="min-w-0 flex-1"><h1 className="text-lg font-semibold text-foreground truncate">Search Results</h1>{!isLoading && <span className="text-sm text-muted-foreground">{totalCount} result{totalCount !== 1 ? "s" : ""}{urlQuery ? ` for "${urlQuery}"` : ""}</span>}</div>
				</div>
				<div className="flex-1 overflow-y-auto">
					{isLoading ? <div className="flex justify-center py-16"><Spinner className="size-6" /></div> : results.length === 0 ? (
						<div className="flex flex-col items-center justify-center py-24 px-6 text-center">
							...
							<p className="text-sm text-muted-foreground max-w-xs">{urlQuery ? `Nothing matched "${urlQuery}". Try different keywords or check your spelling.` : "Enter a search term to find emails by subject, sender, or content."}</p>
							{urlQuery && <p className="text-xs text-muted-foreground mt-3 max-w-sm">Tip: Use operators like <code className="bg-accent px-1 rounded">from:name</code>, <code className="bg-accent px-1 rounded">is:unread</code>, <code className="bg-accent px-1 rounded">has:attachment</code>, <code className="bg-accent px-1 rounded">before:2025-01-01</code></p>}
						</div>
					) : (
						<div>{results.map((email) => {
							...
							return (
								<div key={email.id} role="button" tabIndex={0} onClick={() => handleRowClick(email)} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); handleRowClick(email); } }} className={`group flex items-center gap-3 w-full text-left cursor-pointer transition-colors border-b border-border px-4 py-2.5 md:px-5 md:py-3 ${isPanelOpen ? "md:px-4 md:py-2.5" : ""} ${isSelected ? "bg-accent" : "hover:bg-accent"}`}>
									<div className="w-2.5 shrink-0 flex justify-center">{!email.read && <div className="h-2 w-2 rounded-full bg-primary" />}</div>
									<div className="min-w-0 flex-1">
										<div className="flex items-center gap-2"><span className={`truncate text-sm ${!email.read ? "font-semibold text-foreground" : "text-foreground/70"}`}>{highlightTerms(email.sender.split("@")[0], urlQuery)}</span>{folderName && <Badge variant="outline">{folderDisplayName(folderName)}</Badge>}<span className="text-sm text-muted-foreground shrink-0 ml-auto">{formatListDate(email.date)}</span></div>
				...
				{totalCount > SEARCH_PAGE_SIZE && <div className="flex justify-center py-3 border-t border-border shrink-0"><AppPagination page={currentPage} pageCount={Math.ceil(totalCount / SEARCH_PAGE_SIZE)} onPageChange={setPage} /></div>}
			</>
		</MailboxSplitView>
	);
}
