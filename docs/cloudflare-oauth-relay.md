...
1. Add the receiving domains to **Domains** in Agentic Inbox.
2. Open **Domains → Cloudflare accounts**.
3. Select domains belonging to one Cloudflare account.
4. Select **Authorize with Cloudflare**.
5. Choose the account and approve the requested permissions.

After the callback, Agentic Inbox:

1. Verifies every selected zone belongs to the same authorized account.
2. Generates an Ed25519 key pair in Worker memory.
3. Uploads the relay module through the Workers Scripts API.
4. stores the private key only as a secret on that relay Worker.
5. Stores only the public key in the central Domain Config Durable Object.
6. Saves the existing catch-all configuration for each zone.
7. Points each catch-all rule to the new relay Worker.
8. Revokes the short-lived Cloudflare OAuth access token.
